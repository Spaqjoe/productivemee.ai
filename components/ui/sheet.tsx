...
export function Sheet({ open, onOpenChange, children, className, overlayClassName, side = "right", showCloseButton = true }: SheetProps) {
                ...
                >
                    {showCloseButton && (
                        <button
                            onClick={() => onOpenChange(false)}
                            className={cn(
                                "absolute top-4 z-50 rounded-full bg-black/40 p-2 text-card-foreground backdrop-blur transition-opacity hover:bg-black/60 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:ring-offset-transparent",
                                isLeft ? "left-4" : "right-4"
                            )}
                        >
